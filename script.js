...
function showProduct(id){
  ...
  const action = typeof p.price === "number"
    ? `<button type="button" class="btn primary add-cart-btn" onclick="submitSelection(${p.id}, true)" aria-label="Ajouter ${p.name} au panier">Ajouter au panier</button>`
    : `<button type="button" class="btn primary add-cart-btn" onclick="submitSelection(${p.id}, false)" aria-label="Demander le prix de ${p.name} sur WhatsApp">Demander le prix sur WhatsApp</button>`;
...
function submitSelection(id, addToBasket){
  ...
  const selection = {size:sizeEl ? sizeEl.textContent.trim() : "", color:colorEl ? colorEl.textContent.trim() : ""};
...
document.getElementById("modalClose").onclick=()=>{document.getElementById("productModal").classList.remove("show");document.body.classList.remove("modal-open")};
